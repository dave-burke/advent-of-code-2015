def step(int[][] lights, String instruction){
    String[] words = instruction.split(' ').findAll { it != "turn" && it != "through" };
    
    String state = words[0];
    int a,b,x,y;
    (a,b) = words[1].split(',').collect{Integer.valueOf(it)};
    (x,y) = words[2].split(',').collect{Integer.valueOf(it)};
    
    println("${state} from ${[a,b]} to ${[x,y]}");
    for(int i = a; i <= x; i++){
        for(int j = b; j <= y; j++){
            switch(state){
                case "on":
                ...
                case "toggle":
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("${state} is not a valid instruction");
            }
        }
    }
}

def step2(int[][] lights, String instruction){
    String[] words = instruction.split(' ').findAll { it != "turn" && it != "through" };
    ...
    for(int i = a; i <= x; i++){
        for(int j = b; j <= y; j++){
            switch(state){
                ...
                case "off":
                    lights[i][j]--;
                    lights[i][j] = lights[i][j] < 0 ? 0 : lights[i][j];
                    break;
                case "toggle":
                    lights[i][j] += 2;
                    break;
